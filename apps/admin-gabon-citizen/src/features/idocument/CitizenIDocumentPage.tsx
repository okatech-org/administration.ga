...
/**
 * CitizenIDocumentPage — coffre-fort personnel du citoyen.
 *
 * Rendue côté citizen-web via `app/my-space/idocument/page.tsx` (wrapper
 * Next.js minimal). Le code est extrait de la route pour séparer la logique
 * métier de la définition de route et faciliter la réutilisation future
 * (vue admin du coffre d'un citoyen depuis le back-office, par ex.).
 *
 * Conceptuellement différent de l'iDocument back-office/agent-web : ici on
 * gère le coffre PERSONNEL d'un utilisateur
 * (`api.functions.documentVault.getMyVault`), organisé par TYPE de document
 * (passeport, ID, naissance, …) et par "smart folders" consulaires
 * (identity_civil, official_docs), sans concept d'org.
 */
export default function CitizenIDocumentPage() {
  const { t } = useTranslation()
