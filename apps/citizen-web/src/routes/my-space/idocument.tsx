...
function IDocumentPage() {
	...
	const handleDelete = useCallback(async (doc: VaultDocument) => {
		try {
			await softDeleteDocMut({ documentId: doc._id });
	...
	const handleRestore = useCallback(async (doc: VaultDocument) => {
		try {
			await restoreDocMut({ documentId: doc._id });
	...
	const allFolders = useMemo(() => {
		...
		const systemFolderEntries = SYSTEM_FOLDERS.map((sf) => {
			let count = 0;
			if (sf.id === "__all") count = docs.filter((d) => (d as any).deletedAt === undefined).length;
			else if (sf.id === "__trash") count = docs.filter((d) => (d as any).deletedAt !== undefined).length;
	...
	const currentFiles = useMemo(() => {
		...
		switch (currentFolderId) {
			case "__all":
				// All docs, exclure les soft-deleted
				docs = docs.filter((d) => (d as any).deletedAt === undefined);
				break;
			case "__trash":
				// Documents dans la corbeille (soft-deleted)
				docs = docs.filter((d) => (d as any).deletedAt !== undefined);
				break;
			case "__vault":
				// Validated/secured documents (coffre-fort), exclure soft-deleted
				docs = docs.filter((d) => d.status === "validated" && (d as any).deletedAt === undefined);
				break;
			default: {
