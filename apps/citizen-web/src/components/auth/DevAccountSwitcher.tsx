...
function DevAccountSwitcherInner() {
	...
	const draggable = useDraggable("dev-switcher-pos");
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			<div
				...
				className={`${draggable.style ? "" : "fixed bottom-20 left-4 md:bottom-4"} z-9999 touch-none select-none cursor-grab active:cursor-grabbing`}
				{...draggable.handlers}
			>
				<DialogTrigger asChild>
					<button
						type="button"
						className="flex items-center gap-1.5 rounded-full bg-amber-500 px-3 py-2 text-xs font-bold text-black shadow-lg transition-all hover:bg-amber-400 hover:scale-105 active:scale-95"
						title="Dev Account Switcher"
						onClick={(e) => { if (draggable.isDragged) e.preventDefault(); }}
					>
