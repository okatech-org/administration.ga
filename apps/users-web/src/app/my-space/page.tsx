	...
	return (
		<div className="flex flex-col h-full overflow-hidden relative">
			...
			{activeAlerts.length > 0 && (
				<Link href="/my-space/settings?tab=dossier" className="flex lg:hidden items-center gap-2.5 mt-3 px-3 py-2.5 rounded-xl bg-rose-500/10 border border-rose-500/15 hover:bg-rose-500/15 transition-colors">
										...
										{contacts?.phone && (
											<div className="flex items-center gap-1.5 mt-2 text-xs text-muted-foreground">
												...
												<Button asChild size="icon" variant="ghost" className="h-5 w-5 rounded-full hover:bg-foreground/[0.06] shrink-0 ml-auto">
													<Link href="/my-space/profile/edit"><Pencil className="h-2.5 w-2.5 text-muted-foreground" /></Link>
												</Button>
											</div>
										)}
									</div>
									...
									<div className="w-full mt-4 bg-muted rounded-lg p-2.5">
										<div className="flex items-center gap-2.5 text-sm font-medium">
											...
											<Button asChild size="icon" variant="ghost" className="h-6 w-6 rounded-full hover:bg-foreground/[0.06] shrink-0">
												<Link href="/my-space/profile/edit"><Pencil className="h-3 w-3 text-muted-foreground" /></Link>
											</Button>
										</div>
									</div>
								)}
								...
								<div className="p-4 flex flex-col">
									...
									<div className="flex flex-col gap-2.5">
										...
										{p?.passportInfo?.expiryDate && (() => {
											...
											return (
												<Link href="/my-space/services-demarches" className="bg-rose-500/[0.06] rounded-xl p-3.5 border border-rose-500/10 flex flex-col hover:bg-rose-500/10 transition-colors">
								...
								<div className="p-4">
									<div className="flex items-center justify-between mb-3">
										...
										<Button asChild variant="ghost" size="sm" className="h-5 w-5 p-0 hover:bg-foreground/[0.06]">
											<Link href="/my-space/cv"><Pencil className="h-3 w-3 text-muted-foreground" /></Link>
										</Button>
									</div>
									{cvData ? (
										<>
								...
								<div className="p-3 lg:p-4 flex flex-col">
									...
									<div className="flex overflow-x-auto gap-3 pb-2.5 citizen-scrollbar snap-x">
										{children.map((child: any) => {
											...
											return (
												<Link key={child._id} href={`/my-space/children/${child._id}`} className="flex items-center gap-3 bg-muted p-2.5 rounded-lg hover:bg-muted transition-colors min-w-[85%] md:min-w-[220px] flex-1 shrink-0 snap-start pr-4">
								...
								<div className="p-4">
									...
									<div className="flex overflow-x-auto gap-2.5 pb-1.5 citizen-scrollbar snap-x disable-scrollbars">
										{activeAlerts.map((alert, idx) => (
											<Link key={idx} href="/my-space/settings?tab=dossier" className={cn("flex items-center gap-3 p-2.5 rounded-lg shrink-0 snap-start hover:opacity-80 transition-opacity",
							...
							<div className="p-4 flex-1 flex flex-col">
								...
								{posts && posts.length > 0 ? (
									<div className="space-y-2.5 flex-1 overflow-y-auto citizen-scrollbar pr-1">{posts.slice(0, 2).map((post: any) => (
										<Link key={post._id} href={`/news/${post.slug}`} className="block group bg-muted p-3 rounded-lg hover:bg-muted transition-colors">
							...
							<FlatCard className="flex-[2] min-h-0">
								<div className="p-3 flex flex-col h-full">
									...
									<div className="grid grid-cols-2 gap-2 flex-1 min-h-0">
										{latestRequest ? (
											<Link href={`/my-space/requests/${latestRequest.reference || latestRequest._id}`} className="flex flex-col gap-2 p-2.5 rounded-xl bg-amber-500/15 dark:bg-amber-500/10 hover:bg-amber-500/25 transition-colors">
												<div className="flex items-center gap-2">
											...
											<Link href="/my-space/services-demarches" className="flex flex-col gap-2 p-2.5 rounded-xl bg-amber-500/15 dark:bg-amber-500/10">
												<div className="flex items-center gap-2">
													<div className="p-1 rounded-md bg-amber-500/10 shrink-0"><FileText className="h-4 w-4 text-amber-600 dark:text-amber-400" /></div>
										...
										<Link href="/services" className="flex flex-col items-center justify-center gap-1.5 p-2.5 rounded-xl hover:bg-muted transition-colors text-muted-foreground">
							...
							<FlatCard className="flex-[2] min-h-0">
								<div className="p-3 flex flex-col h-full">
									...
									<div className="grid grid-cols-2 gap-2 flex-1 min-h-0">
										...
										<Link href="/my-space/iagenda" className="flex flex-col items-center justify-center gap-1.5 p-2.5 rounded-xl hover:bg-muted transition-colors text-muted-foreground">
											<div className="h-7 w-7 rounded-full bg-foreground/[0.06] dark:bg-foreground/[0.12] flex items-center justify-center">
												<Plus className="h-3.5 w-3.5" />
											</div>
							...
							<div className="p-4 flex-1 flex flex-col">
								...
								{posts && posts.length > 0 ? (
									<div className="space-y-3 flex-1">
										{posts.map((post: any) => (
											<Link key={post._id} href={`/news/${post.slug}`} className="block group bg-muted p-4 rounded-xl hover:bg-muted/80 transition-colors">
