...
export default function UserRequestDetail() {
	...
	const sections = useMemo(() => {
		...
		if (formSchema?.sections && formSchema.sections.length > 0) {
			// Merge contact_info + residence_address + homeland_address into one "Coordonnees" section
			...
			// Build merged "Coordonnees" section
			...
			if (coordRows.length > 0) {
				...
				templateSections.splice(pos, 0, {
					id: "coordinates",
					title: lang === "fr" ? "Coordonnees" : "Contact & Addresses",
					rows: coordRows,
				});
			}
	...
	const handleSubmit = async (data: Record<string, unknown>) => {
		...
		try {
			...
			if (requiresAppointment) {
				...
				router.push(`/my-space/requests/${request._id}/appointment`);
			} else {
				toast.success(t("request.submitted_success"), {
									...
									>
										<CardContent className="flex flex-col gap-4 py-4">
											...
											<Button
												...
												className="w-full"
												onClick={() =>
													router.push(`/my-space/appointments/${apt._id}`)
												}
											>
