...
export const Route = createFileRoute("/_app/iprofil")({
	component: IProfilPage,
});
...
const LANG_NAMES: Record<string, string> = {
	...
	ar: "Arabe", zh: "Chinois", de: "Allemand", it: "Italien",
	ru: "Russe", ja: "Japonais", ko: "Coréen", sw: "Swahili",
};

// ─── Main Page ─────────────────────────────────────────────────
function IProfilPage() {
	...
	const data = useConvexQuery(api.functions.diplomaticProfile.getMyDiplomaticProfile, {});

	const { mutateAsync: updateProfile, isPending: isSaving } = useConvexMutationQuery(
								...
								<div className="space-y-1.5">
									{(position.moduleAccess as any[]).map((ma: any) => (
										<div key={ma.moduleCode} className="flex items-center justify-between rounded-lg border px-3 py-1.5">
											<span className="text-xs font-medium capitalize">{ma.moduleCode.replace(/_/g, " ")}</span>
											<Badge
												variant="outline"
												className={cn("text-[10px]",
													ma.accessLevel === "admin" ? "text-emerald-600 border-emerald-300"
														: ma.accessLevel === "editor" ? "text-amber-600 border-amber-300"
															: "text-blue-600 border-blue-300",
												)}
											>
												{ma.accessLevel === "admin" ? "Admin" : ma.accessLevel === "editor" ? "Éditeur" : "Lecteur"}
											</Badge>
										</div>
									))}
								</div>
								...
								<div className="space-y-2">
									{profile.previousPostings.map((posting: any, i: number) => (
										<div key={i} className="flex items-start gap-3 rounded-lg border px-3 py-2.5">
											...
											<div className="flex-1 min-w-0">
												<p className="text-sm font-medium">{posting.position}</p>
												<p className="text-xs text-muted-foreground">{posting.orgName}</p>
												<p className="text-[10px] text-muted-foreground mt-0.5">
													{new Date(posting.startDate).getFullYear()}
													{posting.endDate ? ` — ${new Date(posting.endDate).getFullYear()}` : " — présent"}
													{" · "}{posting.country}
												</p>
											</div>
										</div>
							...
							{profile?.officialSignature?.title ? (
								<div className="rounded-lg border p-3 text-center">
									...
									{profile.officialSignature.imageStorageId && (
										<p className="text-[10px] text-muted-foreground mt-1">Image de signature configurée</p>
									)}
								</div>
							) : (
...
function CredentialCard({ title, subtitle, icon, hasData }: { title: string; subtitle: string; icon: string; hasData: boolean }) {
	return (
		<div className={cn("flex items-center gap-3 rounded-lg border px-3 py-2.5", hasData ? "border-border" : "border-border/50 opacity-60")}>
