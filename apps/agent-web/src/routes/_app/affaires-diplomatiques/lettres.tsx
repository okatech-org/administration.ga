...
function LettresPhase() {
  ...
  const search = useSearch({ from: "/_app/affaires-diplomatiques/lettres" });
  ...
  useEffect(() => {
    if (search.targetId && !showDraftDialog) {
      setSelectedTargetId(search.targetId);
      if (search.planId) {
        setSelectedPlanId(search.planId);
      }
      // Ouvrir le dialog automatiquement
      setShowDraftDialog(true);
    }
  }, [search.targetId, search.planId]); // eslint-disable-line react-hooks/exhaustive-deps
