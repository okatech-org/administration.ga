...
import {
	Archive, Search, FileText, Lock, Landmark, Users2, Scale,
	Building2, Shield, ChevronRight, LayoutGrid, List,
	RotateCcw, Trash2, CalendarClock,
	AlertTriangle, CheckCircle2, Timer, Infinity as InfinityIcon,
	X, MoreHorizontal, Eye, History,
} from "lucide-react";
...
type ArchiveStatusFilter = "all" | "active" | "expiring" | "expired" | "perpetual";
...
const CATEGORY_ICON_MAP: Record<string, React.ElementType> = {
	fiscal: Landmark, social: Users2, juridique: Scale, consulaire: Building2, coffre: Lock,
};

const CATEGORY_COLOR_MAP: Record<string, { text: string; bg: string; border: string }> = {
...
const STATUS_CFG: Record<string, { label: string; icon: React.ElementType; color: string }> = {
	...
	expiring: { label: "Expiration proche", icon: AlertTriangle, color: "text-amber-400" },
	expired: { label: "Expiré", icon: Timer, color: "text-red-400" },
	perpetual: { label: "Perpétuel", icon: InfinityIcon, color: "text-violet-400" },
};
...
function ArchiveStatusBadge({ status }: { status: string }) {
	...
	const Icon = cfg.icon;
	return (
		<span className={cn("inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-muted/60 border border-border/30", cfg.color)}>
	...
	return (
		...
		>
			...
			<span className={cn(
				...
				count > 0 ? `${colors.bg} ${colors.text}` : "bg-muted/40 text-muted-foreground",
			)}>
				{count} doc{count !== 1 ? "s" : ""}
			</span>
		</motion.button>
...
function ArchivedDocCard({
	...
	const catColors = CATEGORY_COLOR_MAP[
		ARCHIVE_CATEGORIES.find((c) => c.slug === doc.archiveCategorySlug)?.color ?? "violet"
	] ?? CATEGORY_COLOR_MAP.violet;

	const title = doc.label ?? doc.files?.[0]?.filename ?? "Document";
	const archivedDate = doc.archivedAt ? new Date(doc.archivedAt).toLocaleDateString("fr-FR") : "—";
	...
	return (
		...
		>
			{/* Icone fichier */}
			<div className={cn("h-9 w-9 rounded-lg flex items-center justify-center shrink-0", catColors.bg)}>
			...
			<div className="flex-1 min-w-0">
				...
				{doc.archiveNote && (
					<p className="text-[10px] text-muted-foreground/70 mt-1 truncate italic">{doc.archiveNote}</p>
				)}
			</div>
				...
				{showMenu && (
					<>
						...
						<div className="absolute right-0 top-8 z-50 w-44 border border-border/60 rounded-lg bg-popover shadow-xl py-1">
							<button onClick={() => { onView(); setShowMenu(false); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-muted transition-colors">
								<Eye className="h-3 w-3" />Voir le document
							</button>
							<button onClick={() => { onRestore(); setShowMenu(false); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-muted transition-colors text-emerald-400">
								<RotateCcw className="h-3 w-3" />Restaurer
							</button>
							<div className="border-t border-border/40 my-1" />
							<button onClick={() => { onDelete(); setShowMenu(false); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-muted transition-colors text-red-400">
...
function IArchivePage() {
	...
	const { data: archivedDocs = [] } = useAuthenticatedConvexQuery(
		...
		activeOrgId ? { orgId: activeOrgId, categorySlug: selectedCategory ?? undefined, search: search || undefined } : "skip",
	);
	...
	const filteredDocs = useMemo(() => {
		...
		return (archivedDocs as any[]).filter((d: any) => d.archiveStatus === statusFilter);
	...
	return (
		...
		>
			<div className="max-w-7xl mx-auto px-4 py-6 space-y-5">
					...
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-xs text-muted-foreground">
								{stats?.total ?? 0} documents archivés · {stats?.expiring ?? 0} en expiration
							</p>
						</div>
					</div>
					...
					<div className="flex items-center gap-3">
						{stats && stats.expiring > 0 && (
							<div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-amber-500/10 border border-amber-500/20">
								...
								<span className="text-[10px] font-medium text-amber-400">{stats.expiring} expiration(s)</span>
							</div>
						)}
						{stats && stats.expired > 0 && (
							<div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-red-500/10 border border-red-500/20">
								<Timer className="h-3 w-3 text-red-400" />
								<span className="text-[10px] font-medium text-red-400">{stats.expired} expiré(s)</span>
							</div>
						)}
					</div>
				</motion.div>
				...
				<motion.div variants={fadeUp} className="flex items-center gap-3 flex-wrap">
								...
								{tab.value !== "all" && stats && (
									<span className="ml-1 text-muted-foreground/60">
										({tab.value === "active" ? stats.active
											: tab.value === "expiring" ? stats.expiring
					...
					<div className="flex items-center gap-0.5 bg-muted/30 rounded-lg p-0.5 border border-border/30">
						<button
							...
							className={cn("p-1.5 rounded-md transition-colors", viewMode === "grid" ? "bg-background shadow-sm" : "hover:bg-muted")}
						>
							<LayoutGrid className="h-3.5 w-3.5" />
						</button>
						<button
							onClick={() => setViewMode("list")}
							className={cn("p-1.5 rounded-md transition-colors", viewMode === "list" ? "bg-background shadow-sm" : "hover:bg-muted")}
						>
				...
				{/* ── Breadcrumb ── */}
				<motion.div variants={fadeUp} className="flex items-center gap-1.5 text-xs text-muted-foreground">
					<button onClick={() => setSelectedCategory(null)} className="hover:text-foreground transition-colors font-medium">
						Archives
					</button>
					{selectedCategory && (
						<>
							...
							<span className="text-foreground font-medium">
								{ARCHIVE_CATEGORIES.find((c) => c.slug === selectedCategory)?.name}
							</span>
						</>
					)}
				</motion.div>
						...
						{ARCHIVE_CATEGORIES.map((cat) => {
							...
							return (
								...
								>
									...
									{cat.name}
									<span className="text-[10px] opacity-70">({categoryCounts[cat.slug] ?? 0})</span>
								</button>
							);
						})}
					</motion.div>
				...
				<motion.div variants={stagger}>
					{(filteredDocs as any[]).length === 0 ? (
						...
						>
							...
							<p className="text-sm font-medium text-muted-foreground">Aucun document archivé</p>
				...
				{(auditLog as any[]).length > 0 && !selectedCategory && (
					<motion.div variants={fadeUp} className="mt-6">
						<div className="flex items-center gap-2 mb-3">
							...
							<h3 className="text-xs font-semibold text-muted-foreground">Activité récente</h3>
						</div>
						<div className="space-y-1">
							{(auditLog as any[]).slice(0, 8).map((entry: any) => (
								<div key={entry._id} className="flex items-center gap-2 text-[10px] text-muted-foreground py-1 px-2 rounded-md hover:bg-muted/30">
									<span className={cn(
										"h-1.5 w-1.5 rounded-full shrink-0",
										entry.action === "archive" ? "bg-violet-400"
											: entry.action === "restore" ? "bg-emerald-400"
												: entry.action === "permanent_delete" ? "bg-red-400"
													: "bg-blue-400",
									)} />
									<span className="font-medium text-foreground/70">{entry.actorName ?? "Agent"}</span>
									<span>
										{entry.action === "archive" ? "a archivé"
											: entry.action === "restore" ? "a restauré"
												: entry.action === "permanent_delete" ? "a supprimé"
													: entry.action === "policy_update" ? "a modifié la politique"
														: entry.action === "category_change" ? "a changé la catégorie"
					...
					>
						...
						>
							...
							<p className="text-xs text-muted-foreground mb-5">
								Le document sera supprimé définitivement et ne pourra plus être restauré. Confirmez-vous cette action ?
							</p>
