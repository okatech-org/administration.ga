...
function FolderContextMenu({ itemId, itemName, itemType, onShare, onInfo, onTransmit, onManageAccess, onCreateSubfolder, onDelete, isSystem }: {
	itemId: string; itemName: string; itemType: "folder" | "correspondence"; onShare?: (id: string, type: string) => void; onInfo?: (id: string) => void; onTransmit?: (id: string) => void; onManageAccess?: (id: string) => void; onCreateSubfolder?: (id: string) => void; onDelete?: (id: string) => void; isSystem?: boolean;
}) {
						...
						<div className="flex items-center gap-3 pt-1 flex-wrap">
							{item.status && (
								<span className="text-[10px] text-muted-foreground/80 flex items-center gap-1">
									<span className={cn("h-1.5 w-1.5 rounded-full", STATUS_CFG[item.status as CorrStatus]?.dot || "bg-muted-foreground")} />
									{statusLabel}
								</span>
							)}
					...
					{itemType === "correspondence" && item.type && (
						<div className="rounded-xl bg-card border border-border/50 p-3 space-y-2">
							...
							<div className="flex gap-2 flex-wrap">
								<span className={cn("text-[9px] px-2 py-1 rounded-md font-medium border", CORRESPONDENCE_TYPE_CONFIG[item.type as CorrespondenceType]?.color)}>
...
function ICorrespondancePage() {
	...
	const [isSubmittingTypeDemarche, setIsSubmittingTypeDemarche] = useState(false);
	...
	const filteredDossiers = useMemo(() => {
		...
		if (dossierSearch) {
			...
			items = items.filter((d: any) => d.reference?.toLowerCase().includes(q) || d.typeDemarcheLabel?.toLowerCase().includes(q) || d.status?.toLowerCase().includes(q));
		}
	...
	const filteredTypeDemarches = useMemo(() => {
		...
		if (typeDemarcheSearch) {
			...
			items = items.filter((t: any) => t.code?.toLowerCase().includes(q) || t.label?.toLowerCase().includes(q) || t.category?.toLowerCase().includes(q));
		}
		return items;
	...
	const handleSubmitTypeDemarche = useCallback(async () => {
		if (!typeDemarcheForm.code.trim() || !typeDemarcheForm.label.trim() || !activeOrgId) return;
		...
		try {
			if (editingTypeDemarche) {
				await updateTypeDemarcheMutation.mutateAsync({
					...
					description: typeDemarcheForm.description.trim() ? { fr: typeDemarcheForm.description.trim() } : undefined,
	...
	}, [typeDemarcheForm, activeOrgId, editingTypeDemarche, createTypeDemarcheMutation, updateTypeDemarcheMutation]);

	const handleDeactivateTypeDemarche = useCallback(async (id: string) => {
		try {
	...
	return (
		<motion.div initial="hidden" animate="visible" variants={stagger} className="space-y-5 max-w-[1400px] mx-auto p-4">
			...
			<motion.div variants={fadeUp}>
				<div className="flex items-center gap-1 border border-border/50 rounded-xl bg-card p-1">
					{([
						...
						{ key: "audit" as ActiveTab, label: "Journal d'audit", icon: Activity },
						{ key: "statistiques" as ActiveTab, label: "Statistiques", icon: BarChart3 },
					]).map((tab) => (
						<button
							key={tab.key}
						...
						>
							...
							{tab.key === "dossiers" && (dossiers as any[]).length > 0 && (
								<span className="ml-1 text-[9px] opacity-60">({(dossiers as any[]).length})</span>
							)}
							{tab.key === "types-demarches" && (typeDemarches as any[]).length > 0 && (
								<span className="ml-1 text-[9px] opacity-60">({(typeDemarches as any[]).length})</span>
							)}
						</button>
					))}
				</div>
					...
					<div className="w-full max-w-md border border-border/50 shadow-2xl bg-popover rounded-2xl" onClick={(e) => e.stopPropagation()}>
						...
						<div className="p-5 space-y-3">
							<div className="space-y-2">
								...
								<input placeholder="Ex: Notes Verbales 2026" value={newFolderName} onChange={(e) => setNewFolderName(e.target.value)} className="w-full h-9 px-3 text-xs bg-muted/50 border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/30" autoFocus onKeyDown={(e) => { if (e.key === "Enter" && newFolderName.trim()) handleCreateFolder(); }} />
							</div>
						</div>
						<div className="px-5 py-3 border-t border-border/50 flex justify-end gap-2">
						...
						<div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
							<div className="grid grid-cols-2 gap-3">
								<div className="col-span-2 space-y-1.5">
									...
									<input placeholder="Ex: Note verbale — Demande de visa" value={newCorr.title} onChange={(e) => setNewCorr((p) => ({ ...p, title: e.target.value }))} className="w-full h-9 px-3 text-xs bg-muted/50 border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/30" autoFocus />
								</div>
								<div className="space-y-1.5">
									<label className="text-xs font-medium">Type *</label>
									<select value={newCorr.type} onChange={(e) => setNewCorr((p) => ({ ...p, type: e.target.value as CorrespondenceType }))} className="w-full h-9 px-3 text-xs bg-muted/50 border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/30">
										{Object.entries(CORRESPONDENCE_TYPE_CONFIG).map(([v, c]) => (
								...
								<div className="space-y-1.5">
									...
									<select value={newCorr.priority} onChange={(e) => setNewCorr((p) => ({ ...p, priority: e.target.value as Priority }))} className="w-full h-9 px-3 text-xs bg-muted/50 border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/30">
								...
								<div className="col-span-2 space-y-1.5">
									...
									<input placeholder="Ex: Ambassade du Gabon en France" value={newCorr.senderName} onChange={(e) => setNewCorr((p) => ({ ...p, senderName: e.target.value }))} className="w-full h-9 px-3 text-xs bg-muted/50 border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/30" />
								</div>
								<div className="col-span-2 space-y-1.5">
								...
								<div className="space-y-1.5">
									<label className="text-xs font-medium">Email destinataire</label>
									<input type="email" placeholder="contact@mae.ga" value={newCorr.recipientEmail} onChange={(e) => setNewCorr((p) => ({ ...p, recipientEmail: e.target.value }))} className="w-full h-9 px-3 text-xs bg-muted/50 border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/30" />
								</div>
								<div className="col-span-2 space-y-1.5">
									<label className="text-xs font-medium">Note / Commentaire</label>
									<textarea rows={3} placeholder="Commentaire interne ou objet de la correspondance..." value={newCorr.comment} onChange={(e) => setNewCorr((p) => ({ ...p, comment: e.target.value }))} className="w-full px-3 py-2 text-xs bg-muted/50 border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/30 resize-none" />
								</div>
							</div>
						</div>
						<div className="px-5 py-3 border-t border-border/50 flex justify-end gap-2">
				...
				<motion.div variants={fadeUp} className="space-y-4">
					...
					<div className="border border-border/50 rounded-xl bg-card p-3">
						<div className="relative max-w-[360px]">
							<Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" />
							<input placeholder="Rechercher un type de d\u00e9marche..." value={typeDemarcheSearch} onChange={(e) => setTypeDemarcheSearch(e.target.value)} className="w-full h-8 pl-8 text-xs bg-muted/50 border border-border/50 rounded-lg px-3 focus:outline-none focus:ring-1 focus:ring-primary/30" />
						</div>
					</div>
					...
					<div className="border border-border/50 rounded-xl overflow-hidden bg-card">
						...
						{/* Rows */}
						{filteredTypeDemarches.length > 0 ? filteredTypeDemarches.map((td: any) => (
							<div key={td._id} className="grid grid-cols-12 gap-2 px-4 py-2.5 border-b border-border/30 hover:bg-muted/30 transition-colors group">
								<div className="col-span-2 flex items-center">
									<span className="text-xs font-mono text-muted-foreground">{td.code}</span>
								</div>
								<div className="col-span-3 flex items-center">
									<span className="text-xs font-medium truncate">{td.label}</span>
								</div>
								<div className="col-span-2 flex items-center">
									<span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20">{td.category || "\u2014"}</span>
								</div>
								<div className="col-span-1 flex items-center">
									<span className="text-xs text-muted-foreground">{td.piecesRequises?.length ?? 0}</span>
								</div>
								<div className="col-span-1 flex items-center">
									<span className="text-xs text-muted-foreground">{td.etapes?.length ?? 0}</span>
								</div>
								<div className="col-span-1 flex items-center">
									{td.isActive !== false ? (
										<span className="text-[9px] px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400 border border-emerald-500/20 font-medium">Actif</span>
									) : (
										<span className="text-[9px] px-2 py-0.5 rounded-full bg-zinc-500/15 text-zinc-400 border border-zinc-500/20 font-medium">Inactif</span>
									)}
								</div>
								<div className="col-span-2 flex items-center gap-1">
									<button onClick={() => handleOpenTypeDemarcheForm(td)} className="h-7 w-7 rounded-md flex items-center justify-center hover:bg-muted transition-colors text-muted-foreground hover:text-foreground" title="Modifier">
										<Edit3 className="h-3.5 w-3.5" />
									</button>
									{td.isActive !== false && (
										<button onClick={() => handleDeactivateTypeDemarche(td._id)} className="h-7 w-7 rounded-md flex items-center justify-center hover:bg-muted transition-colors text-muted-foreground hover:text-red-400" title="D\u00e9sactiver">
							...
							<div className="py-12 text-center text-sm text-muted-foreground">Aucun type de d\u00e9marche trouv\u00e9</div>
						)}
					</div>
							...
							<div className="w-full max-w-lg border border-border/50 shadow-2xl bg-popover rounded-2xl" onClick={(e) => e.stopPropagation()}>
								<div className="px-5 pt-5 pb-3 border-b border-border/50">
									<div className="flex items-center gap-2 text-sm font-semibold">
										...
										{editingTypeDemarche ? "Modifier le type de d\u00e9marche" : "Nouveau type de d\u00e9marche"}
									</div>
								</div>
								<div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
									<div className="grid grid-cols-2 gap-3">
										<div className="space-y-1.5">
											...
											<input placeholder="Ex: PASSEPORT" value={typeDemarcheForm.code} onChange={(e) => setTypeDemarcheForm((p) => ({ ...p, code: e.target.value }))} disabled={!!editingTypeDemarche} className="w-full h-9 px-3 text-xs bg-muted/50 border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/30 disabled:opacity-50 font-mono" autoFocus />
										</div>
										<div className="space-y-1.5">
											<label className="text-xs font-medium">Cat\u00e9gorie *</label>
											<select value={typeDemarcheForm.category} onChange={(e) => setTypeDemarcheForm((p) => ({ ...p, category: e.target.value }))} className="w-full h-9 px-3 text-xs bg-muted/50 border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/30">
										...
										<div className="col-span-2 space-y-1.5">
											...
											<input placeholder="Ex: Demande de passeport" value={typeDemarcheForm.label} onChange={(e) => setTypeDemarcheForm((p) => ({ ...p, label: e.target.value }))} className="w-full h-9 px-3 text-xs bg-muted/50 border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/30" />
										</div>
										<div className="col-span-2 space-y-1.5">
											<label className="text-xs font-medium">Description</label>
											<textarea rows={3} placeholder="Description de la d\u00e9marche..." value={typeDemarcheForm.description} onChange={(e) => setTypeDemarcheForm((p) => ({ ...p, description: e.target.value }))} className="w-full px-3 py-2 text-xs bg-muted/50 border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/30 resize-none" />
										</div>
										<div className="space-y-1.5">
											<label className="text-xs font-medium">D\u00e9lai de traitement (jours)</label>
											<input type="number" min={1} value={typeDemarcheForm.delaiTraitementJours} onChange={(e) => setTypeDemarcheForm((p) => ({ ...p, delaiTraitementJours: Number.parseInt(e.target.value) || 30 }))} className="w-full h-9 px-3 text-xs bg-muted/50 border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary/30" />
										</div>
									</div>
								</div>
								<div className="px-5 py-3 border-t border-border/50 flex justify-end gap-2">
				...
				<motion.div variants={fadeUp} className="space-y-4">
					...
					<div className="border border-border/50 rounded-xl bg-card p-3">
						<div className="flex flex-wrap items-center gap-2">
							<div className="relative flex-1 min-w-[200px] max-w-[360px]">
								<Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" />
								<input placeholder="Rechercher un dossier..." value={dossierSearch} onChange={(e) => setDossierSearch(e.target.value)} className="w-full h-8 pl-8 text-xs bg-muted/50 border border-border/50 rounded-lg px-3 focus:outline-none focus:ring-1 focus:ring-primary/30" />
							</div>
							<div className="h-6 w-px bg-border/50 hidden sm:block" />
							<div className="flex items-center gap-1">
								{[
									...
									<button key={f.value} onClick={() => setDossierStatusFilter(f.value)} className={cn("px-2.5 py-1 rounded-full text-[11px] font-medium transition-all", dossierStatusFilter === f.value ? "bg-violet-500/20 text-violet-300 ring-1 ring-violet-500/30" : "text-muted-foreground hover:bg-muted")}>
										{f.label}
									</button>
								))}
							</div>
					...
					<div className="border border-border/50 rounded-xl overflow-hidden bg-card">
						...
						{filteredDossiers.length > 0 ? filteredDossiers.map((d: any) => (
							<div key={d._id} className="grid grid-cols-12 gap-2 px-4 py-2.5 border-b border-border/30 hover:bg-muted/30 transition-colors">
								<div className="col-span-2 flex items-center">
									<span className="text-xs font-mono text-muted-foreground">{d.reference || d._id?.slice(-8)}</span>
								</div>
								<div className="col-span-3 flex items-center">
									<span className="text-xs font-medium truncate">{d.typeDemarcheLabel || "\u2014"}</span>
								</div>
								<div className="col-span-2 flex items-center">
									<span className={cn("text-[10px] border inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full font-medium",
										d.status === "brouillon" ? "bg-zinc-500/15 text-zinc-400 border-zinc-500/20" :
										d.status === "soumis" ? "bg-blue-500/15 text-blue-400 border-blue-500/20" :
										d.status === "en_cours" ? "bg-amber-500/15 text-amber-400 border-amber-500/20" :
										d.status === "complete" ? "bg-emerald-500/15 text-emerald-400 border-emerald-500/20" :
										d.status === "rejete" ? "bg-red-500/15 text-red-400 border-red-500/20" :
								...
								<div className="col-span-2 flex items-center">
									{d.priorite && d.priorite !== "normal" && (
										<span className={cn("text-[9px] px-2 py-1 rounded-md font-medium", d.priorite === "urgent" ? "text-red-400" : "text-amber-400")}>
											{d.priorite}
										</span>
									)}
									{(!d.priorite || d.priorite === "normal") && <span className="text-xs text-muted-foreground/50">Normal</span>}
								</div>
								<div className="col-span-1 flex items-center">
									<span className="text-xs text-muted-foreground">{d.currentStepIndex != null ? `${d.currentStepIndex + 1}/${d.totalSteps || "?"}` : "\u2014"}</span>
								</div>
								<div className="col-span-2 flex items-center text-xs text-muted-foreground">
									<Clock className="h-2.5 w-2.5 mr-1" />
									{d._creationTime ? new Date(d._creationTime).toLocaleDateString("fr-FR") : "\u2014"}
								</div>
							</div>
						)) : (
							<div className="py-12 text-center text-sm text-muted-foreground">Aucun dossier trouv\u00e9</div>
						)}
					</div>
				</motion.div>
			)}
			...
			{activeTab === "audit" && (
				<motion.div variants={fadeUp}>
					<div className="border border-border/50 rounded-xl bg-card p-12 text-center">
			...
			{activeTab === "statistiques" && (
				<motion.div variants={fadeUp}>
					<div className="border border-border/50 rounded-xl bg-card p-12 text-center">
						...
						<h3 className="text-lg font-semibold mb-1">Statistiques</h3>
						<p className="text-sm text-muted-foreground">Coming soon</p>
					</div>
				</motion.div>
			)}
