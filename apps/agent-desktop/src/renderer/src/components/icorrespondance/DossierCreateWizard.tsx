/**
 * DossierCreateWizard — Multi-step wizard for creating a dossier de procedure.
 * Steps: 1) Select type -> 2) Fill metadata -> 3) Submit
 */
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-2xl border border-border/50 shadow-2xl bg-popover rounded-2xl max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-5 py-3 border-b border-border/50 flex items-center gap-3 shrink-0">
          {["Type de demarche", "Details", "Recapitulatif"].map((label, idx) => (
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {step === 0 && (
            <div className="space-y-4">
              <p className="text-xs text-muted-foreground">
                Selectionnez le type de demarche administrative.
              </p>
              {categories.length > 1 && (
              ...
              <div className="space-y-2">
                {filteredTypes.map((td) => (
                  <button
                    key={td._id}
                    onClick={() => { setSelectedType(td); setStep(1); }}
                    className={cn(
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        ...
                        <span className="text-[9px] text-muted-foreground/40">
                          {td.piecesRequises.length} pieces - {td.etapesParcours.length} etapes
                        </span>
                      </div>
                    </div>
                ...
                {filteredTypes.length === 0 && (
                  <div className="text-center py-8 text-xs text-muted-foreground/50">
                    Aucun type de demarche disponible
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60">
                  Pieces requises ({selectedType.piecesRequises.filter((p) => p.required).length} obligatoires)
                </p>
                <div className="space-y-1">
                  {selectedType.piecesRequises.map((p) => (
                    <div key={p.code} className="flex items-center gap-2 text-xs py-1">
                      <div className="h-1.5 w-1.5 rounded-full bg-muted-foreground/30" />
                      <span className={p.required ? "font-medium" : "text-muted-foreground"}>{p.label.fr}</span>
              ...
              <div className="space-y-2">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60">
                  Parcours ({selectedType.etapesParcours.length} etapes)
                </p>
                <div className="flex items-center gap-1 flex-wrap">
                  {[...selectedType.etapesParcours].sort((a, b) => a.ordre - b.ordre).map((e, idx) => (
                    <div key={e.code} className="flex items-center gap-1">
                      ...
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted border border-border/50">{e.label.fr}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {step === 2 && selectedType && (
            <div className="space-y-4">
              <p className="text-xs text-muted-foreground">
                Verifiez les informations avant de creer le dossier.
              </p>
              <div className="border border-border/50 rounded-xl p-4 space-y-3 bg-card">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <p className="text-[9px] text-muted-foreground/60 uppercase">Pieces</p>
                    <p className="text-xs font-medium">{selectedType.piecesRequises.length} pieces</p>
                  </div>
                </div>
                {notes && (
                  <div>
              ...
              <div className="border border-amber-500/20 rounded-lg p-3 bg-amber-500/5 text-xs text-amber-400">
                Le dossier sera cree en statut <strong>brouillon</strong>. Vous pourrez ensuite y deposer les pieces justificatives et le soumettre pour traitement.
              </div>
            </div>
          )}
        </div>
