...
import {
	Mail, Search, Clock, Lock, Users2,
	Building2, Folder, FolderOpen, FolderPlus,
	Loader2, X,
	MoreHorizontal, Share2, Send, Info, KeyRound, Tag,
	Trash2, User,
	LayoutGrid, List, Columns3, ChevronRight, Plus, BarChart3,
} from "lucide-react";
import { cn } from "../../lib/utils";

// ─── Real components (adapted from agent-web) ─────────
import { DossierList } from "./DossierList";
import { DossierDetail } from "./DossierDetail";
import { DossierCreateWizard } from "./DossierCreateWizard";
import { CorrespondanceDashboard as Dashboard } from "./CorrespondanceDashboard";
import { NewCorrespondanceWizard } from "./NewCorrespondanceWizard";
import { CorrespondanceDossier } from "./CorrespondanceDossier";
import { CorrespondanceDetail } from "./CorrespondanceDetail";
...
/* VaultFileCard removed — unused, can be restored from git history */
...
function FolderContextMenu({ itemId, itemType, onShare, onInfo, onTransmit, onManageAccess, onDelete, isSystem }: {
...
export function ICorrespondancePage() {
	// ─── Org context ────────────────────────────────────────
	const { orgId } = useOrg();
	...
	const [transmitTargetName] = useState("");
	...
	const handleCreateFolder = useCallback(async () => {
		if (!newFolderName.trim() || !orgId) return;
	...
	}, [newFolderName, orgId, currentFolderId, createFolderMutation]);
