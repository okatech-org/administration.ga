...
export const Route = createFileRoute("/my-space/children/$childId")({
	component: ChildDashboardPage,
});

// ─── Helpers ─────────────────────────────────────────────────
const GENDER_LABELS: Record<string, string> = { male: "Garçon", female: "Fille", M: "Garçon", F: "Fille" };
...
const ROLE_LABELS: Record<string, string> = { father: "Père", mother: "Mère", guardian: "Tuteur", other: "Autre" };
...
function ChildDashboardPage() {
	...
	if (!child) {
		return (
			<div className="flex flex-col items-center justify-center h-full gap-4">
				...
				<Button asChild variant="outline" size="sm"><Link to="/my-space"><ArrowLeft className="mr-1.5 h-3.5 w-3.5" />Retour</Link></Button>
			</div>
		);
	}
	...
	const DOC_ENTRIES = [
		...
		{ key: "residencePermit", label: "Titre de séjour", icon: Shield },
		{ key: "addressProof", label: "Justificatif domicile", icon: MapPin },
		{ key: "photo", label: "Photo d'identité", icon: User },
	];
		...
		<div className="flex flex-col h-full overflow-hidden bg-background">
			...
			<div className="px-2 pt-2 shrink-0 flex items-center gap-2">
				<Button asChild variant="ghost" size="sm" className="h-7 px-2 text-xs">
					<Link to="/my-space"><ArrowLeft className="h-3.5 w-3.5 mr-1" />Mon espace</Link>
				</Button>
					...
					<div className="lg:col-span-3 flex flex-col gap-2 min-h-0 overflow-y-auto">
						...
						<Card className="shrink-0 bg-pink-500/5 border-pink-500/10 shadow-sm relative overflow-hidden">
							<CardContent className="p-2.5 flex flex-col items-center text-center">
								...
								<div className="w-full mt-3 space-y-1 text-left bg-background/50 rounded p-2 border border-pink-500/10">
									{identity?.birthPlace && <div className="flex items-center gap-1.5 text-[10px]"><MapPin className="h-3 w-3 text-muted-foreground shrink-0" /><span className="truncate">Né(e) à {identity.birthPlace}</span></div>}
									...
									{c.countryOfResidence && <div className="flex items-center gap-1.5 text-[10px]"><MapPin className="h-3 w-3 text-muted-foreground shrink-0" /><span className="truncate">Réside en {lbl(COUNTRY_LABELS, c.countryOfResidence)}</span></div>}
								</div>
							</CardContent>
						</Card>
						...
						<Card className="shrink-0">
							<CardContent className="p-2.5">
								...
								) : <p className="text-[10px] text-muted-foreground text-center py-2">Non renseigné</p>}
							</CardContent>
						</Card>
					</div>

					{/* ─── COL 2: Dossier Enfant — Identité détaillée + Passeport (6/12) ─── */}
						...
						<Card className="flex-1 shadow-sm flex flex-col overflow-hidden">
							<CardContent className="p-0 flex flex-col h-full">
								...
								<div className="flex-1 overflow-y-auto p-3 space-y-4">
									{/* Identité */}
									<div>
										<p className="text-[9px] font-semibold uppercase tracking-wider text-muted-foreground/60 mb-2">Identité</p>
										<div className="grid grid-cols-2 sm:grid-cols-3 gap-x-5 gap-y-2">
											<InfoRow label="Nom" value={identity?.lastName} />
											<InfoRow label="Prénom(s)" value={identity?.firstName} />
											...
											{c.countryOfResidence && <InfoRow label="Résidence" value={lbl(COUNTRY_LABELS, c.countryOfResidence)} />}
										</div>
									</div>
									...
									<div>
										<div className="flex items-center gap-2 mb-2">
											...
											{ppExpired && <Badge variant="destructive" className="text-[8px] h-4">Expiré</Badge>}
											{ppSoon && <Badge variant="secondary" className="text-[8px] h-4 bg-amber-100 text-amber-700">Expire bientôt</Badge>}
										</div>
										{pp?.number ? (
											<div className={cn("grid grid-cols-2 sm:grid-cols-4 gap-x-5 gap-y-2 p-3 rounded-lg border", ppExpired ? "border-red-400/30 bg-red-500/5" : ppSoon ? "border-amber-400/30 bg-amber-500/5" : "border-border/50")}>
												<InfoRow label="Numéro" value={pp.number} />
												<InfoRow label="Autorité" value={pp.issueAuthority} />
												<InfoRow label="Délivré le" value={fmtDate(pp.issueDate)} />
										...
										{cc?.cardNumber ? (
											<div className={cn("p-3 rounded-lg border", ccExpired ? "border-red-400/30 bg-red-500/5" : "border-emerald-400/30 bg-emerald-500/5")}>
												<div className="grid grid-cols-3 gap-x-5 gap-y-2">
													...
													<InfoRow label="Délivré le" value={fmtDate(cc.issuedAt)} />
								...
								<div className="space-y-1.5">
									<Button asChild variant="outline" size="sm" className="w-full h-7 text-[10px] justify-start">
										<Link to="/my-space/services-demarches">
											<FileText className="h-3 w-3 mr-1.5" />Démarche pour {firstName}
										</Link>
									</Button>
									<Button asChild variant="outline" size="sm" className="w-full h-7 text-[10px] justify-start">
										<Link to="/my-space/iagenda">
									...
									<Button asChild variant="ghost" size="sm" className="w-full h-7 text-[10px] justify-start text-muted-foreground">
										<Link to="/my-space">
											<ArrowLeft className="h-3 w-3 mr-1.5" />Retour à mon espace
										</Link>
									</Button>
								</div>
							</CardContent>
