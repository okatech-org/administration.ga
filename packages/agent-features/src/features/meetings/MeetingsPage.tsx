...
export interface MeetingRoomInjectedProps {
	token: string;
	wsUrl: string;
	onDisconnect: () => void;
...
export interface MeetingsPageProps {
	/** Full-featured LiveKit meeting room (agent-web). Injected to avoid bundling
	 * agent-web-specific LiveKit UI inside the shared package. */
	MeetingRoom: ComponentType<MeetingRoomInjectedProps>;
	/** Pre-join screen for a meeting. Injected alongside MeetingRoom. */
	PreJoinScreen: ComponentType<PreJoinScreenInjectedProps>;
}
	...
	const handleExportIcs = useCallback(
		async (meetingId: Id<"meetings">) => {
			try {
				...
				const blob = new Blob([result.ics], {
