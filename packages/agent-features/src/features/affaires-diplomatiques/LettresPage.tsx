...
export default function LettresPhase() {
	...
	const requestDocx = useMutation(
		api.functions.diplomaticAffairs.requestLetterDocxGeneration,
	);
	...
	const eligibleTargets = targets?.filter(
		(t) => t.pipelinePhase === "strategy" || t.pipelinePhase === "outreach",
	);
	...
	useEffect(() => {
		if (targetIdParam && !showDraftDialog) {
		...
		<div className="space-y-4">
			...
			{eligibleTargets &&
				...
				targets.length > 0 && (
					<div className="rounded-lg bg-warning/10 border border-warning/20 p-3 text-xs text-warning">
						Aucune cible en phase "Strategie" ou "Contact". Generez d'abord un
						plan strategique depuis l'onglet Cibles.
					</div>
				)}
			...
			{!letters || letters.length === 0 ? (
				...
				<div className="space-y-2">
					{letters.map((letter) => {
						const status = STATUS_LABEL[letter.status] ?? STATUS_LABEL.draft;
						return (
							<FlatCard key={letter._id}>
					...
					draftResult ? (
						<div className="space-y-3">
							<div className="rounded-lg bg-success/5 border border-success/20 p-4">
								<p className="text-sm font-medium mb-1">{draftResult.subject}</p>
