...
export default function RapportsPhase() {
			...
			{!reports || reports.length === 0 ? (
				<div className="flex flex-col items-center justify-center py-16 text-center">
					...
					<p className="text-sm text-muted-foreground max-w-md mb-6">
						Preparez des rapports d'activite pour le President, les Ministres et
						votre hierarchie. Importez vos comptes-rendus de reunion pour
						enrichir le rapport.
					</p>
					<Button
						className="gap-1.5"
				...
				<div className="space-y-2">
					{reports.map((report) => {
						const status = STATUS_LABEL[report.status] ?? STATUS_LABEL.draft;
						return (
							<FlatCard key={report._id}>
							...
							priorities.sourceDocuments.length > 0 && (
								<div className="rounded-lg bg-muted/50 p-2 space-y-1">
									<p className="text-[10px] font-medium text-muted-foreground">
										{priorities.sourceDocuments.length} document(s) dans la base
										de connaissances — seront utilises comme contexte
									</p>
					...
					compileResult ? (
						<div className="space-y-3">
							<div className="rounded-lg bg-destructive/5 border border-destructive/20 p-4">
								<p className="text-sm font-medium mb-1">{compileResult.title}</p>
							...
							<div className="grid grid-cols-4 gap-2">
								...
								<div className="rounded-lg bg-muted/50 p-2 text-center">
									...
									<p className="text-[9px] text-muted-foreground">Contactees</p>
								</div>
								<div className="rounded-lg bg-muted/50 p-2 text-center">
									...
									<p className="text-[9px] text-muted-foreground">Reunions</p>
								</div>
								<div className="rounded-lg bg-muted/50 p-2 text-center">
									...
									<ul className="space-y-0.5">
										{compileResult.recommendations.map((r, i) => (
											...
											>
												...
												{typeof r === "string"
													? r
													: (r as { title?: string }).title ?? JSON.stringify(r)}
											</li>
										))}
									</ul>
								</div>
