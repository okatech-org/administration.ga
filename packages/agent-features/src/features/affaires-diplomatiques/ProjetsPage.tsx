...
function ProjectCard({ project, orgId }: { project: any; orgId: Id<"orgs"> | null }) {
	...
	const handleGenerateDocx = async () => {
		...
		try {
			...
			toast.success(
				"Document DOCX en cours de generation. Il apparaitra dans iDocument.",
			);
		} catch (error) {
	...
	const handleGeneratePdf = async () => {
		...
		try {
			...
			toast.success(
				"PDF en cours de generation. Il apparaitra dans le dossier cible.",
			);
		} catch (error) {
				...
				{project.objectives.length > 0 && (
					<div className="space-y-1.5">
						...
						{project.objectives
							...
							.map(
								(
									obj: { title: string; status: string },
									i: number,
								) => (
									<div
										key={i}
										className="flex items-start gap-2 text-xs ml-5"
									>
										<div
											className={cn(
...
export default function ProjetsPhase() {
	...
	const [projectType, setProjectType] = useState<string>("cooperation_agreement");
	...
	useEffect(() => {
		if (targetIdParam && !showProjectDialog) {
			setSelectedTargetId(targetIdParam);
			setShowProjectDialog(true);
		}
	...
	const handleStructureProject = async () => {
		...
		try {
			...
			toast.success(`Projet "${result.title}" structure avec succes`);
		} catch (error) {
			...
			{eligibleTargets &&
				...
				targets.length > 0 && (
					<div className="rounded-lg bg-warning/10 border border-warning/20 p-3 text-xs text-warning">
						Aucune cible en phase avancee. Progressez dans le pipeline (lettres,
						rapports) avant de structurer un projet.
					</div>
				)}
			...
			{!projects || projects.length === 0 ? (
				<div className="flex flex-col items-center justify-center py-16 text-center">
					...
					<p className="text-sm text-muted-foreground max-w-md mb-6">
						Apres validation par la haute autorite, etablissez des contrats et
