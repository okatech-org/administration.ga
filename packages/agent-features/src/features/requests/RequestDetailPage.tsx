...
// Heavy agent-web-only components are injected via props so the package
// remains host-agnostic. See shim in apps/agent-web/src/app/(app)/requests/[reference]/page.tsx
...
export interface RequestActionModalProps {
	...
	// Intentionally `any` so the host can narrow to its own FormSchema type.
	// The package doesn't care about the shape — it just forwards the value.
	formSchema?: any;
...
export interface RequestDetailPageProps {
	...
	RequestActionModal: ComponentType<RequestActionModalProps>;
	OfficialDocumentsSection: ComponentType<OfficialDocumentsSectionProps>;
	UserProfilePreviewCard: ComponentType<UserProfilePreviewCardProps>;
}
...
export default function RequestDetailPage({
	...
	const sections = useMemo(() => {
		...
		for (const sId of ["contact_info", "residence_address", "homeland_address"] as const) {
			...
			if (schemaSection) {
				coordFields.push(
					...buildFields(sId, formDataObj[sId], schemaSection.fields ?? []),
				);
			}
		}
		if (coordFields.length > 0) {
			// Insert after family_info or at the start
