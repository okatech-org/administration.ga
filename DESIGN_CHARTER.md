...
### 2.4 Surfaces Neumorphiques (citizen-web)

| Surface | Light Mode | Dark Mode | Variable |
|---------|------------|-----------|----------|
| Page | #EFEFEF | #141414 | `--neu-surface` |
| Carte | #F5F5F5 | #1E1E1E | `--neu-surface-card` |
| Sidebar | #EFEFEF | #121212 | `--neu-surface-sidebar` |
...
```tsx
className="rounded-xl bg-card p-0 overflow-hidden border flat-card-border"
```
...
.gabon-stripe {
  ...
  background: linear-gradient(90deg,
    #009E60 33.33%, #FCD116 33.33%,
    #FCD116 66.66%, #3A75C4 66.66%
  );
  border-radius: 2px;
}

/* Bande verticale */
.gabon-stripe-vertical { width: 3px; /* meme gradient en 180deg */ }
```
...
```css
.bg-gabon-green-tint  { background: rgba(0, 158, 96, 0.08); }  /* Dark: 0.15 */
.bg-gabon-yellow-tint { background: rgba(252, 209, 22, 0.08); } /* Dark: 0.12 */
.bg-gabon-blue-tint   { background: rgba(58, 117, 196, 0.08); } /* Dark: 0.15 */
```
...
::-webkit-scrollbar-thumb { background: oklch(0.5 0 0 / 0.15); border-radius: 9999px; }
/* Firefox */
* { scrollbar-width: thin; scrollbar-color: oklch(0.5 0 0 / 0.15) transparent; }
```
...
--secondary-foreground: oklch(0.40 0 0);
