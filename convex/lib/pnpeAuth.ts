 ...
 *
 * Le rôle PNPE est stocké dans la table `pnpeStaffAssignments` (cf.
 * `convex/schemas/pnpe/pnpeStaffAssignments.ts`) — séparée de
 * `memberships` qui reste un join générique userId↔orgId.
 *
 * Usage :
...
export async function requirePnpeRole(
  ...
  const hasRole = assignments.some(
    (a) => allowedRoles.includes(a.pnpeRole) && a.isActive,
  );
  if (!hasRole) {
    throw new Error(
